...
',
  },
  {
    ...
    answer: '문제에 구성요소 미제공/ 예시: 커널/ 사용자 인터페이스 등, * 유저인터페이스(UserInterface): 사용자와의 통신담당, -텍스트기반(셸/ Shell): 키보드와 모니터 화면을 활용한 텍스트메시지 -그래픽 사용자 인터페이스(GUI: Graphical user interface): 여러가지 입력장치(마우스/ 터치스크린)를 활용하여 조작 *윈도우관리자(Window manager): 응용프로그램과 화면간의 매핑담당,  •커널(kernel): 컴퓨터가 필요로 하는 가장 기본적인 기능수행, -기본적인 필수컴퓨터 기능수행, -일정관리자(scheduler)와 실행관리자(dispatcher), -메모리(memory)관리자: 주기억장치의 사용관장 / •주기억장치에서 공간 할당을 관장 /•페이징(paging): 주기억장치와 대용량저장장치 사이에서 데이터 블록(페이지) 이동 / •가상메모리(virtual memory): 페이징에 의해 만들어진 ‘가상적인’ 큰 기억공간, -파일(file)관리자: 파일정보관리/ •대용량 저장장치에 저장된 모든 파일정보유지/•디렉터리(또는폴더): 파일들과 하위디렉터리들을 포함하는묶음. 계층구조형성 / •디렉토리경로: 디렉터리안의 디렉터리로 이어지는 연결사슬(/ \로표현), -장치(device) 관리자: 주변장치와 통신제어',
  },
  {
    chapter: '3장',
