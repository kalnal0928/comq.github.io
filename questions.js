const questions = [
  ...
  },
  {
    ...
    answer: '문제에 방식 미제공/ 예시: 일괄처리/ 대화식/ 시분할 등, 일괄(batch)처리: • 한 번에 한 프로그램만 실행 / • 선입 선출(First-In, First-Out) 방식의 작업 큐(queue) /• 작업 실행 도중에 사용자의 개입 불가, 대화식 처리: • 원격 단말기를 통해 시스템과 사용자가 대화 가능/• 사용자는 단말기로 입력하고 시스템의 응답을 확인함/ • 현재의 클라우드 서비스 사용법과 동일, 멀티프로그래밍: • CPU는 한 번에 여러 개의 프로세스 처리/ • I/O 연산을 기다리는 대신, 다른 프로세스 실행/ • 하나의 컴퓨터를 다수의 사용자가 동시에 사용/ • 시간을 구간으로 분할(시분할, time-sharing)/ • 한 작업은 한 번에 한 구간 동안만 처리',
  },
  {
    chapter: '3장',
