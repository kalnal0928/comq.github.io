const questions = [
  ...
  },
  {
    ...
    answer: '낮은 저장 용량 (대용량 저장장치는 저장 용량이 큼),  * 주기억장치대비 장점–, 낮은휘발성/ 큰저장용량/ 낮은비용/ 탈착가능, * 대용량저장장치의단점 , - 자기/광학방식:, 느린데이터접근시간(기계적동작), – 기계장치:, 고장가능성높음, – 플래시/SSD:, 속도수명한계(반도체내구성',
  },
  {
    chapter: '1장',
  ...
  },
  {
    ...
    answer: '논리적 자리 이동 (Logical shift), 부호비트를 신경쓰지 않는 이동은 -> 논리시프트, 보후비트를 보존하는(변경이 없는 자리 이동) 이동은 -> 산술시프트',
  },
  {
    chapter: '2장',
  ...
  },
  {
    ...
    answer: '문제에 장치 미제공/ 예시: 중앙처리장치 → CPU, *중앙처리장치(Central Processing Unit, CPU), -데이터조작을제어하는컴퓨터회로, -연산(arithmetic/logic) 장치: 데이터에대한연산수행, -제어(control) 장치: 컴퓨터의동작조정, -레지스터(register) 장치: CPU내에서정보임시저장, * 버스(bus)-CPU와 주기억장치간에 비트패턴을 전송하는 전선묶음 ~-셀의주소/ 읽기~쓰기신호/ 데이터전송, * 주기억장치(main memory)-컴퓨터에서수치·명령·자료등을 기억하는 하드웨어장치-CPU 회로를변경하는 대신 주기억장치 내용을 변경함으로써 프로그램 변경가능(프로그램내장개념)',
  },
  {
    chapter: '2장',
    question: '다음 기계어 타입은?',
  ...
  },
  {
    ...
    answer: '문제에 방식 미제공/ 예시: 일괄처리/ 대화식/ 시분할 등, 일괄(batch)처리: • 한 번에 한 프로그램만 실행 / • 선입 선출(First-In First-Out) 방식의 작업 큐(queue) /• 작업 실행 도중에 사용자의 개입 불가, 대화식 처리: • 원격 단말기를 통해 시스템과 사용자가 대화 가능/ • 사용자는 단말기로 입력하고 시스템의 응답을 확인함/ • 현재의 클라우드 서비스 사용법과 동일, 멀티프로그래밍: • CPU는 한 번에 여러 개의 프로세스 처리/ • I/O 연산을 기다리는 대신 다른 프로세스 실행/ • 하나의 컴퓨터를 다수의 사용자가 동시에 사용/ • 시간을 구간으로 분할(시분할 time-sharing)/ • 한 작업은 한 번에 한 구간 동안만 처리, 멀티태스킹: • 한 사용자가 다수의 작업을 수행/ • 단일 사용자 시스템에 적용된 시분할 기법, 다중프로세서: • 1개 이상의 CPU 활용/ • 각 CPU 마다 프로세스 할당하여 처리/ • 여러 개의 프로세스로 시분할/멀티태스킹 제공/ • 부하균형(load balancing): 작업을 여러 프로세스에 잘 할당하여 효율적으로 사용/ • 스케일링(scaling): 프로세스 수에 맞추어 작업을 작은 단위로 분할, 임베디드: • 기기 제어를 위한 특정 기능을 수행하는 컴퓨터 시스템 (개인용 컴퓨터는 특정되지 않는 일반적 기능 수행)/ • 의료기기/ 차량용 전자기기/ 가전제품/ 휴대폰 등/ • 배터리 절약/ 실시간 일정 충족/ 자율적/지속적 운영 등의 요건 충족 필요',
  },
  {
    chapter: '3장',
